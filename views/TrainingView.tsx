  ...
  return (
    <div className="flex h-full overflow-hidden bg-background-light dark:bg-background-dark">
      <section className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {/* Player de Vídeo */}
        <div className="aspect-video w-full bg-black rounded-3xl overflow-hidden shadow-2xl relative border border-slate-800 flex items-center justify-center group">
           <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
           <button 
              className="w-20 h-20 bg-primary text-white rounded-full flex items-center justify-center shadow-2xl transform transition hover:scale-110 z-10"
              onClick={() => alert('Iniciando aula...')}
           >
              <span className="material-icons-outlined text-5xl ml-1">play_arrow</span>
           </button>
           <div className="absolute bottom-0 left-0 right-0 p-8 flex items-center space-x-4 z-10">
              <div className="h-1.5 flex-1 bg-white/20 rounded-full overflow-hidden">
                 <div className="h-full bg-primary w-1/3"></div>
              </div>
              <span className="text-sm font-bold text-white">04:00 / 12:00</span>
           </div>
        </div>
